...
static void wolfTPM2_CopyKeyFromBlob(WOLFTPM2_KEY *key,
                                     const WOLFTPM2_KEYBLOB *keyBlob);
static void wolfTPM2_CopyNvPublic(TPMS_NV_PUBLIC *out,
...
static int wolfTPM2_Init_ex(TPM2_CTX *ctx, TPM2HalIoCb ioCb, void *userCtx,
  ...
  printf("TPM2: Caps 0x%08x, Did 0x%04x, Vid 0x%04x, Rid 0x%2x \n", ctx->caps,
         ctx->did_vid >> 16, ctx->did_vid & 0xFFFF, ctx->rid);
#endif
  ...
  /* Do full self-test (Chips such as ATTPM20 require this before some
   * operations) */
...
/* Single-shot API for testing access to hardware and optionally return
 * capabilities */
...
int wolfTPM2_Init(WOLFTPM2_DEV *dev, TPM2HalIoCb ioCb, void *userCtx) {
  ...
  spi = L4Re::chkcap(L4Re::Env::env()->get_cap<SPI>("spi"),
                     "failed to get spi cap");
...
int wolfTPM2_CreateAuthSession_EkPolicy(WOLFTPM2_DEV *dev,
  ...
  rc = wolfTPM2_StartSession(dev, tpmSession, NULL, NULL, TPM_SE_POLICY,
                             TPM_ALG_NULL);
  if (rc == TPM_RC_SUCCESS) {
#ifdef DEBUG_WOLFTPM
...
int wolfTPM2_RSA_Salt(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *tpmKey,
  ...
  if (rc == 0) {
    ...
    rc = wc_RsaPublicEncrypt_ex(
        ...
        encSalt->size,   /* outLen length of buffer available to hold encrypted
                            msg */
        &rsaKey,         /* key initialized RSA key struct */
        &rng,            /* rng initialized WC_RNG struct */
        WC_RSA_OAEP_PAD, /* type type of padding to use (WC_RSA_OAEP_PAD or
                            WC_RSA_PKCSV15_PAD) */
        hashType, /* hash type of hash to use (choices can be found in hash.h)
                   */
        mgf,      /* mgf type of mask generation function to use */
        (byte *)label, /* label an optional label to associate with encrypted
                          message */
...
int wolfTPM2_EncryptSalt(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *tpmKey,
                         StartAuthSession_In *in, TPM2B_AUTH *bindAuth,
                         TPM2B_DIGEST *salt) {
  int rc;
...
int wolfTPM2_StartSession(WOLFTPM2_DEV *dev, WOLFTPM2_SESSION *session,
                          WOLFTPM2_KEY *tpmKey, WOLFTPM2_HANDLE *bind,
                          TPM_SE sesType, int encDecAlg) {
  int rc;
  ...
  rc = TPM2_GetNonce(authSesIn.nonceCaller.buffer, authSesIn.nonceCaller.size);
  if (rc < 0) {
#ifdef DEBUG_WOLFTPM
  ...
  if (keyIn.size > 0) {
    ...
    rc = TPM2_KDFa(authSesIn.authHash, &keyIn, "ATH", &authSesOut.nonceTPM,
                   &authSesIn.nonceCaller, session->handle.auth.buffer,
...
int wolfTPM2_CreatePrimaryKey(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *key,
                              ...
                              TPMT_PUBLIC *publicTemplate, const byte *auth,
                              int authSz) {
  int rc;
...
int wolfTPM2_ChangeAuthKey(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *key,
                           WOLFTPM2_HANDLE *parent, const byte *auth,
                           int authSz) {
  int rc;
...
int wolfTPM2_CreateKey(WOLFTPM2_DEV *dev, WOLFTPM2_KEYBLOB *keyBlob,
  ...
  XMEMSET(&createOut, 0,
          sizeof(createOut)); /* make sure pub struct is zero init */
...
int wolfTPM2_CreateLoadedKey(WOLFTPM2_DEV *dev, WOLFTPM2_KEYBLOB *keyBlob,
  ...
  /* clear output key buffer */
  XMEMSET(keyBlob, 0, sizeof(WOLFTPM2_KEYBLOB));
  XMEMSET(&createLoadedOut, 0,
          sizeof(createLoadedOut)); /* make sure pub struct is zero init */
...
int wolfTPM2_SensitiveToPrivate(TPM2B_SENSITIVE *sens, TPM2B_PRIVATE *priv,
                                TPMI_ALG_HASH nameAlg, TPM2B_NAME *name,
  ...
  if (innerWrap) {
    ...
    rc = TPM2_KDFa(innerAlg, (TPM2B_DATA *)&sens->sensitiveArea.seedValue,
                   "STORAGE", (TPM2B_NONCE *)name, NULL, symKey.buffer,
                   symKey.size);
    if (rc != symKey.size) {
#ifdef DEBUG_WOLFTPM
    ...
    if (rc == 0) {
      rc = wc_AesSetKey(&enc, symKey.buffer, symKey.size, ivField.buffer,
                        AES_ENCRYPTION);
      /* Encryption in-place */
      if (rc == 0)
        rc = wc_AesCfbEncrypt(&enc, &packet.buf[innerSz], &packet.buf[innerSz],
                              sensSz);
      wc_AesFree(&enc);
    }
#else
  ...
  if (outerWrap) {
    ...
    rc = TPM2_KDFa(outerAlg, (TPM2B_DATA *)&sens->sensitiveArea.seedValue,
                   "INTEGRITY", NULL, NULL, hmacKey.buffer, hmacKey.size);
    if (rc != hmacKey.size) {
#ifdef DEBUG_WOLFTPM
    ...
    if (rc == 0)
      rc =
          wc_HmacFinal(&hmac_ctx, &packet.buf[sizeof(word16) + sizeof(word16)]);
...
int wolfTPM2_ImportPrivateKey(WOLFTPM2_DEV *dev, const WOLFTPM2_KEY *parentKey,
                              WOLFTPM2_KEYBLOB *keyBlob,
                              const TPM2B_PUBLIC *pub, TPM2B_SENSITIVE *sens) {
  int rc;
  Import_In importIn;
  ...
  if (dev == NULL || keyBlob == NULL || pub == NULL || sens == NULL) {
    return BAD_FUNC_ARG;
  }
  ...
  rc = wolfTPM2_SensitiveToPrivate(sens, &importIn.duplicate,
                                   pub->publicArea.nameAlg, &name, parentKey,
                                   &importIn.symmetricAlg, &importIn.inSymSeed);
  if (rc != TPM_RC_SUCCESS) {
#ifdef DEBUG_WOLFTPM
...
int wolfTPM2_LoadPrivateKey(WOLFTPM2_DEV *dev, const WOLFTPM2_KEY *parentKey,
                            WOLFTPM2_KEY *key, const TPM2B_PUBLIC *pub,
...
int wolfTPM2_LoadRsaPublicKey_ex(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *key,
                                 const byte *rsaPub, word32 rsaPubSz,
                                 word32 exponent, TPMI_ALG_RSA_SCHEME scheme,
  ...
  pub.publicArea.objectAttributes =
      (TPMA_OBJECT_sign | TPMA_OBJECT_decrypt | TPMA_OBJECT_userWithAuth |
       TPMA_OBJECT_noDA);
...
int wolfTPM2_ImportRsaPrivateKey(
    WOLFTPM2_DEV *dev, const WOLFTPM2_KEY *parentKey, WOLFTPM2_KEYBLOB *keyBlob,
...
int wolfTPM2_LoadRsaPrivateKey_ex(
    WOLFTPM2_DEV *dev, const WOLFTPM2_KEY *parentKey, WOLFTPM2_KEY *key,
    const byte *rsaPub, word32 rsaPubSz, word32 exponent, const byte *rsaPriv,
    word32 rsaPrivSz, TPMI_ALG_RSA_SCHEME scheme, TPMI_ALG_HASH hashAlg) {
  int rc;
  WOLFTPM2_KEYBLOB keyBlob;
  ...
  rc = wolfTPM2_ImportRsaPrivateKey(dev, parentKey, &keyBlob, rsaPub, rsaPubSz,
                                    exponent, rsaPriv, rsaPrivSz, scheme,
                                    hashAlg);
  if (rc == 0) {
...
int wolfTPM2_ImportEccPrivateKey(WOLFTPM2_DEV *dev,
                                 const WOLFTPM2_KEY *parentKey,
...
int wolfTPM2_LoadEccPrivateKey(WOLFTPM2_DEV *dev, const WOLFTPM2_KEY *parentKey,
  ...
  XMEMCPY(&keyBlob, key, sizeof(WOLFTPM2_KEY));
  rc = wolfTPM2_ImportEccPrivateKey(dev, parentKey, &keyBlob, curveId, eccPubX,
                                    eccPubXSz, eccPubY, eccPubYSz, eccPriv,
                                    eccPrivSz);
  if (rc == 0) {
    rc = wolfTPM2_LoadKey(dev, &keyBlob, (WOLFTPM2_HANDLE *)&parentKey->handle);
  }
...
int wolfTPM2_ReadPublicKey(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *key,
  ...
  printf("TPM2_ReadPublic Handle 0x%x: pub %d, name %d, qualifiedName %d\n",
         (word32)readPubIn.objectHandle, readPubOut.outPublic.size,
         readPubOut.name.size, readPubOut.qualifiedName.size);
#endif
...
int wolfTPM2_RsaKey_WolfToTpm_ex(WOLFTPM2_DEV *dev,
                                 const WOLFTPM2_KEY *parentKey, RsaKey *wolfKey,
                                 WOLFTPM2_KEY *tpmKey) {
  int rc;
  word32 exponent;
  byte e[sizeof(exponent)];
  ...
  if (parentKey && wolfKey->type == RSA_PRIVATE) {
    ...
    rc = wc_RsaExportKey(wolfKey, e, &eSz, n, &nSz, d, &dSz, p, &pSz, q, &qSz);
    if (rc == 0) {
      exponent = wolfTPM2_RsaKey_Exponent(e, eSz);
      rc = wolfTPM2_LoadRsaPrivateKey(dev, parentKey, tpmKey, n, nSz, exponent,
                                      q, qSz);
    }
...
int wolfTPM2_EccKey_WolfToTpm_ex(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *parentKey,
  ...
  if (parentKey && wolfKey->type == ECC_PRIVATEKEY) {
    ...
    if (rc == 0) {
      rc = wolfTPM2_LoadEccPrivateKey(dev, parentKey, tpmKey, curve_id, qx,
                                      qxSz, qy, qySz, d, dSz);
    }
  } else {
...
int wolfTPM2_EccKey_WolfToPubPoint(WOLFTPM2_DEV *dev, ecc_key *wolfKey,
  ...
  rc = wc_ecc_export_public_raw(wolfKey, pubPoint->point.x.buffer, &xSz,
...
int wolfTPM2_SignHashScheme(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *key,
                            const byte *digest, int digestSz, byte *sig,
                            int *sigSz, TPMI_ALG_SIG_SCHEME sigAlg,
                            TPMI_ALG_HASH hashAlg) {
  int rc;
  Sign_In signIn;
  Sign_Out signOut;
  ...
  if (key->pub.publicArea.type == TPM_ALG_ECC) {
    /* get curve size */
    curveSize =
        wolfTPM2_GetCurveSize(key->pub.publicArea.parameters.eccDetail.curveID);
...
int wolfTPM2_SignHash(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *key, const byte *digest,
                      int digestSz, byte *sig, int *sigSz) {
  ...
  return wolfTPM2_SignHashScheme(dev, key, digest, digestSz, sig, sigSz, sigAlg,
                                 WOLFTPM2_WRAP_DIGEST);
}
...
int wolfTPM2_VerifyHashScheme(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *key,
                              const byte *sig, int sigSz, const byte *digest,
                              int digestSz, TPMI_ALG_SIG_SCHEME sigAlg,
                              TPMI_ALG_HASH hashAlg) {
  int rc;
  ...
  if (key->pub.publicArea.type == TPM_ALG_ECC) {
    ...
    if (curveSize >
        (int)sizeof(verifySigIn.signature.signature.ecdsa.signatureR.buffer))
      return BAD_FUNC_ARG;
  ...
  if (key->pub.publicArea.type == TPM_ALG_ECC) {
    ...
    XMEMCPY(verifySigIn.signature.signature.ecdsa.signatureR.buffer, sig,
            curveSize);
...
int wolfTPM2_VerifyHash(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *key, const byte *sig,
                        int sigSz, const byte *digest, int digestSz) {
...
int wolfTPM2_ECDHGenKey(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *ecdhKey, int curve_id,
  ...
  if (rc == 0) {
    rc = wolfTPM2_CreatePrimaryKey(dev, ecdhKey, TPM_RH_NULL, &publicTemplate,
                                   auth, authSz);
  }
...
/* One time use key */
int wolfTPM2_ECDHEGenKey(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *ecdhKey,
                         int curve_id) {
  int rc;
  EC_Ephemeral_In in;
...
int wolfTPM2_ECDHEGenZ(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *parentKey,
  ...
  if (dev == NULL || parentKey == NULL || ecdhKey == NULL || pubPoint == NULL ||
      out == NULL || outSz == NULL) {
    return BAD_FUNC_ARG;
  }
  ...
  *outSz = outZGen2Ph.outZ2.point.x.size;
  XMEMCPY(out, outZGen2Ph.outZ2.point.x.buffer, outZGen2Ph.outZ2.point.x.size);
...
int wolfTPM2_RsaEncrypt(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *key,
                        TPM_ALG_ID padScheme, const byte *msg, int msgSz,
                        byte *out, int *outSz) {
  int rc;
...
int wolfTPM2_RsaDecrypt(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *key,
                        TPM_ALG_ID padScheme, const byte *in, int inSz,
                        byte *msg, int *msgSz) {
  int rc;
...
int wolfTPM2_ReadPCR(WOLFTPM2_DEV *dev, int pcrIndex, int hashAlg, byte *digest,
  ...
  printf("TPM2_PCR_Read: Index %d, Digest Sz %d, Update Counter %d\n", pcrIndex,
         digestLen, (int)pcrReadOut.pcrUpdateCounter);
...
int wolfTPM2_NVCreateAuth(WOLFTPM2_DEV *dev, WOLFTPM2_HANDLE *parent,
                          WOLFTPM2_NV *nv, word32 nvIndex, word32 nvAttributes,
                          word32 maxSize, const byte *auth, int authSz) {
...
#ifndef WOLFTPM2_NO_WOLFCRYPT
  rc = TPM2_HashNvPublic(&in.publicInfo.nvPublic, (byte *)&nv->handle.name.name,
                         &nv->handle.name.size);
  if (rc != TPM_RC_SUCCESS) {
    return rc;
  }
  ...
  printf("TPM2_NV_DefineSpace: Auth 0x%x, Idx 0x%x, Attribs 0x%d, Size %d\n",
         (word32)in.authHandle, (word32)in.publicInfo.nvPublic.nvIndex,
...
int wolfTPM2_NVCreate(WOLFTPM2_DEV *dev, TPM_HANDLE authHandle, word32 nvIndex,
                      word32 nvAttributes, word32 maxSize, const byte *auth,
                      int authSz) {
  WOLFTPM2_NV nv;
  WOLFTPM2_HANDLE parent;
...
int wolfTPM2_NVWriteAuth(WOLFTPM2_DEV *dev, WOLFTPM2_NV *nv, word32 nvIndex,
                         byte *dataBuf, word32 dataSz, word32 offset) {
...
int wolfTPM2_NVWrite(WOLFTPM2_DEV *dev, TPM_HANDLE authHandle, word32 nvIndex,
                     byte *dataBuf, word32 dataSz, word32 offset) {
  WOLFTPM2_NV nv;
  XMEMSET(&nv, 0, sizeof(nv));
  nv.handle.hndl = (TPM_HANDLE)nvIndex;
...
int wolfTPM2_NVReadAuth(WOLFTPM2_DEV *dev, WOLFTPM2_NV *nv, word32 nvIndex,
                        byte *dataBuf, word32 *pDataSz, word32 offset) {
  int rc = TPM_RC_SUCCESS;
...
int wolfTPM2_NVRead(WOLFTPM2_DEV *dev, TPM_HANDLE authHandle, word32 nvIndex,
                    byte *dataBuf, word32 *pDataSz, word32 offset) {
  WOLFTPM2_NV nv;
  XMEMSET(&nv, 0, sizeof(nv));
  nv.handle.hndl = (TPM_HANDLE)nvIndex;
...
int wolfTPM2_NVReadPublic(WOLFTPM2_DEV *dev, word32 nvIndex,
  ...
  printf("TPM2_NV_ReadPublic: Sz %d, Idx 0x%x, nameAlg %d, Attr 0x%x, "
         ...
         out.nvPublic.size, (word32)out.nvPublic.nvPublic.nvIndex,
         out.nvPublic.nvPublic.nameAlg,
         (word32)out.nvPublic.nvPublic.attributes,
         out.nvPublic.nvPublic.authPolicy.size, out.nvPublic.nvPublic.dataSize,
         out.nvName.size);
#endif
...
int wolfTPM2_HashStart(WOLFTPM2_DEV *dev, WOLFTPM2_HASH *hash,
                       TPMI_ALG_HASH hashAlg, const byte *usageAuth,
                       word32 usageAuthSz) {
  int rc;
...
int wolfTPM2_HashFinish(WOLFTPM2_DEV *dev, WOLFTPM2_HASH *hash, byte *digest,
                        word32 *digestSz) {
  int rc;
...
int wolfTPM2_LoadSymmetricKey(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *key, int alg,
  ...
  XMEMCPY(loadExtIn.inPrivate.sensitiveArea.sensitive.sym.buffer, keyBuf,
          keySz);
  ...
  rc = wolfTPM2_ComputeSymmetricUnique(
      dev, hashAlg, &loadExtIn.inPrivate.sensitiveArea,
...
int wolfTPM2_EncryptDecryptBlock(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *key,
  ...
  encDecIn.inData.size = (encDecIn.inData.size + MAX_AES_BLOCK_SIZE_BYTES - 1) &
...
int wolfTPM2_EncryptDecrypt(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *key,
                            const byte *in, byte *out, word32 inOutSz, byte *iv,
                            word32 ivSz, int isDecrypt) {
  int rc = 0;
  word32 pos = 0, xfer;
  ...
  while (pos < inOutSz) {
    ...
    rc = wolfTPM2_EncryptDecryptBlock(dev, key, &in[pos], &out[pos], xfer, iv,
                                      ivSz, isDecrypt);
    if (rc != TPM_RC_SUCCESS)
      break;
...
int wolfTPM2_LoadKeyedHashKey(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *key,
                              WOLFTPM2_HANDLE *parent, int hashAlg,
                              const byte *keyBuf, word32 keySz,
  ...
  rc = wolfTPM2_GetKeyTemplate_KeyedHash(&createIn.inPublic.publicArea, hashAlg,
                                         YES, NO);
  if (rc != 0) {
    return rc;
  }
...
int wolfTPM2_HmacStart(WOLFTPM2_DEV *dev, WOLFTPM2_HMAC *hmac,
                       WOLFTPM2_HANDLE *parent, TPMI_ALG_HASH hashAlg,
                       const byte *keyBuf, word32 keySz, const byte *usageAuth,
                       word32 usageAuthSz) {
  int rc;
  HMAC_Start_In in;
  HMAC_Start_Out out;
...
int wolfTPM2_HmacFinish(WOLFTPM2_DEV *dev, WOLFTPM2_HMAC *hmac, byte *digest,
                        word32 *digestSz) {
  int rc;
...
static int GetKeyTemplateRSA(TPMT_PUBLIC *publicTemplate, TPM_ALG_ID nameAlg,
                             TPMA_OBJECT objectAttributes, int keyBits,
                             int exponent, TPM_ALG_ID sigScheme,
                             TPM_ALG_ID sigHash) {
...
static int GetKeyTemplateECC(TPMT_PUBLIC *publicTemplate, TPM_ALG_ID nameAlg,
...
int wolfTPM2_GetKeyTemplate_KeyedHash(TPMT_PUBLIC *publicTemplate,
  ...
  publicTemplate->objectAttributes =
      (TPMA_OBJECT_userWithAuth | TPMA_OBJECT_noDA |
       (isSign ? TPMA_OBJECT_sign : 0) | (isDecrypt ? TPMA_OBJECT_decrypt : 0));
...
int wolfTPM2_GetKeyTemplate_RSA_EK(TPMT_PUBLIC *publicTemplate) {
  ...
  ret = GetKeyTemplateRSA(publicTemplate, TPM_ALG_SHA256, objectAttributes,
                          2048, 0, TPM_ALG_NULL, TPM_ALG_NULL);
  if (ret == 0) {
    publicTemplate->authPolicy.size = sizeof(TPM_20_EK_AUTH_POLICY);
    XMEMCPY(publicTemplate->authPolicy.buffer, TPM_20_EK_AUTH_POLICY,
...
int wolfTPM2_GetKeyTemplate_ECC_EK(TPMT_PUBLIC *publicTemplate) {
  ...
  ret = GetKeyTemplateECC(publicTemplate, TPM_ALG_SHA256, objectAttributes,
                          TPM_ECC_NIST_P256, TPM_ALG_NULL, TPM_ALG_NULL);
  if (ret == 0) {
    publicTemplate->authPolicy.size = sizeof(TPM_20_EK_AUTH_POLICY);
    XMEMCPY(publicTemplate->authPolicy.buffer, TPM_20_EK_AUTH_POLICY,
            publicTemplate->authPolicy.size);
...
int wolfTPM2_GetKeyTemplate_RSA_SRK(TPMT_PUBLIC *publicTemplate) {
  ...
  return GetKeyTemplateRSA(publicTemplate, TPM_ALG_SHA256, objectAttributes,
                           2048, 0, TPM_ALG_NULL, TPM_ALG_NULL);
}
...
int wolfTPM2_GetKeyTemplate_ECC_SRK(TPMT_PUBLIC *publicTemplate) {
  ...
  return GetKeyTemplateECC(publicTemplate, TPM_ALG_SHA256, objectAttributes,
                           TPM_ECC_NIST_P256, TPM_ALG_NULL, TPM_ALG_NULL);
}
...
int wolfTPM2_GetKeyTemplate_RSA_AIK(TPMT_PUBLIC *publicTemplate) {
  ...
  ret = GetKeyTemplateRSA(publicTemplate, TPM_ALG_SHA256, objectAttributes,
                          2048, 0, TPM_ALG_RSASSA, TPM_ALG_SHA256);
  if (ret == 0) {
    publicTemplate->parameters.rsaDetail.symmetric.algorithm = TPM_ALG_NULL;
  }
  return ret;
...
int wolfTPM2_GetKeyTemplate_ECC_AIK(TPMT_PUBLIC *publicTemplate) {
  ...
  ret = GetKeyTemplateECC(publicTemplate, TPM_ALG_SHA256, objectAttributes,
                          TPM_ECC_NIST_P256, TPM_ALG_ECDSA, TPM_ALG_SHA256);
  if (ret == 0) {
    publicTemplate->parameters.eccDetail.symmetric.algorithm = TPM_ALG_NULL;
  }
  return ret;
...
int wolfTPM2_GetNvAttributesTemplate(TPM_HANDLE auth, word32 *nvAttributes) {
  ...
  *nvAttributes =
  ...
  if (auth == TPM_RH_PLATFORM) {
    *nvAttributes |= (TPMA_NV_PPWRITE | TPMA_NV_PPREAD);
  }
...
int wolfTPM2_CreateSRK(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *srkKey, TPM_ALG_ID alg,
  ...
  rc = wolfTPM2_CreatePrimaryKey(dev, srkKey, TPM_RH_OWNER, &publicTemplate,
                                 auth, authSz);
...
int wolfTPM2_CreateAndLoadAIK(WOLFTPM2_DEV *dev, WOLFTPM2_KEY *aikKey,
                              TPM_ALG_ID alg, WOLFTPM2_KEY *srkKey,
                              const byte *auth, int authSz) {
  int rc;
  TPMT_PUBLIC publicTemplate;
  ...
  rc = wolfTPM2_CreateAndLoadKey(dev, aikKey, &srkKey->handle, &publicTemplate,
                                 auth, authSz);
...
int wolfTPM2_CreateKeySeal(WOLFTPM2_DEV *dev, WOLFTPM2_KEYBLOB *keyBlob,
                           WOLFTPM2_HANDLE *parent, TPMT_PUBLIC *publicTemplate,
                           const byte *auth, int authSz, const byte *sealData,
                           int sealSize) {
  int rc;
  Create_In createIn;
  Create_Out createOut;
...
static void wolfTPM2_CopyPubT(TPMT_PUBLIC *out, const TPMT_PUBLIC *in) {
  ...
  case TPM_ALG_KEYEDHASH:
    ...
    out->unique.keyedHash.size = in->unique.keyedHash.size;
    if (out->unique.keyedHash.size >
        (UINT16)sizeof(out->unique.keyedHash.buffer)) {
      out->unique.keyedHash.size = (UINT16)sizeof(out->unique.keyedHash.buffer);
    }
  ...
  case TPM_ALG_SYMCIPHER:
    ...
    out->unique.sym.size = in->unique.sym.size;
    if (out->unique.sym.size > (UINT16)sizeof(out->unique.sym.buffer)) {
      out->unique.sym.size = (UINT16)sizeof(out->unique.sym.buffer);
    }
  ...
  case TPM_ALG_RSA:
    ...
    out->parameters.rsaDetail.keyBits = in->parameters.rsaDetail.keyBits;
    out->parameters.rsaDetail.exponent = in->parameters.rsaDetail.exponent;

    out->unique.rsa.size = in->unique.rsa.size;
    if (out->unique.rsa.size > (UINT16)sizeof(out->unique.rsa.buffer)) {
      out->unique.rsa.size = (UINT16)sizeof(out->unique.rsa.buffer);
    }
  ...
  case TPM_ALG_ECC:
    ...
    out->parameters.eccDetail.curveID = in->parameters.eccDetail.curveID;
    out->parameters.eccDetail.kdf = in->parameters.eccDetail.kdf;

    wolfTPM2_CopyEccParam(&out->unique.ecc.x, &in->unique.ecc.x);
    wolfTPM2_CopyEccParam(&out->unique.ecc.y, &in->unique.ecc.y);
    break;
  default:
...
#endif
static int wolfTPM2_HashUpdateCache(WOLFTPM2_HASHCTX *hashCtx, const byte *in,
                                    word32 inSz) {
  int ret = 0;
  ...
  if (hashCtx->cacheBuf == NULL) {
    hashCtx->cacheSz = 0;
    hashCtx->cacheBufSz =
        (inSz + WOLFTPM2_HASH_BLOCK_SZ - 1) & ~(WOLFTPM2_HASH_BLOCK_SZ - 1);
    if (hashCtx->cacheBufSz == 0)
      hashCtx->cacheBufSz = WOLFTPM2_HASH_BLOCK_SZ;
    hashCtx->cacheBuf =
        (byte *)XMALLOC(hashCtx->cacheBufSz, NULL, DYNAMIC_TYPE_TMP_BUFFER);
    if (hashCtx->cacheBuf == NULL) {
      return MEMORY_E;
    }
  }
  /* determine if we need to grow buffer */
    ...
    hashCtx->cacheBufSz =
        (hashCtx->cacheSz + inSz + WOLFTPM2_HASH_BLOCK_SZ - 1) &
...
/* TODO: Add crypto callback RSA keygen support */
      ...
      case RSA_PUBLIC_DECRYPT: {
        ...
        if (!isWolfKeyValid && tlsCtx->rsaKey) {
          ...
          rc = wolfTPM2_RsaEncrypt(
              tlsCtx->dev, tlsCtx->rsaKey, TPM_ALG_NULL, /* no padding */
              info->pk.rsa.in, info->pk.rsa.inLen, info->pk.rsa.out,
              (int *)info->pk.rsa.outLen);
          break;
        }
        ...
        rc = wolfTPM2_RsaKey_WolfToTpm(tlsCtx->dev, info->pk.rsa.key, &rsaPub);
        if (rc != 0) {
    ...
    if (info->pk.type == WC_PK_TYPE_EC_KEYGEN) {
      ...
      rc = wolfTPM2_SignHash(tlsCtx->dev, tlsCtx->eccKey, info->pk.eccsign.in,
                             inlen, sigRS, (int *)&rsLen);
      if (rc == 0) {
        /* Encode ECDSA Header */
        rLen = sLen = rsLen / 2;
        s = &sigRS[rLen];
        rc = wc_ecc_rs_raw_to_sig(r, rLen, s, sLen, info->pk.eccsign.out,
      ...
      /* Decode ECDSA Header */
      rc = wc_ecc_sig_to_rs(info->pk.eccverify.sig, info->pk.eccverify.siglen,
                            r, &rLen, s, &sLen);
      if (rc == 0) {
        /* load public key into TPM */
        ...
        if (rc == 0) {
          ...
          rc = wolfTPM2_VerifyHash(tlsCtx->dev, &eccPub, sigRS, rLen + sLen,
      ...
      rc = wolfTPM2_EccKey_WolfToPubPoint(tlsCtx->dev, info->pk.ecdh.public_key,
                                          &pubPoint);
      if (rc == 0) {
        /* Compute shared secret and compare results */
        rc = wolfTPM2_ECDHGenZ(tlsCtx->dev, tlsCtx->ecdhKey, &pubPoint,
                               info->pk.ecdh.out, (int *)info->pk.ecdh.outlen);
      }
    ...
    if (info->cipher.aescbc.aes) {
      ...
      rc = wolfTPM2_LoadSymmetricKey(tlsCtx->dev, &symKey, TPM_ALG_CBC,
                                     (byte *)aes->devKey, aes->keylen);
      if (rc == 0) {
        /* perform symmetric encrypt/decrypt */
        rc = wolfTPM2_EncryptDecrypt(
            tlsCtx->dev, &symKey, info->cipher.aescbc.in,
            info->cipher.aescbc.out, info->cipher.aescbc.sz, (byte *)aes->reg,
    ...
    if (info->hash.in != NULL) { /* Update */
      ...
      if (rc == 0) {
        if (hashCtx && (hashFlags & WC_HASH_FLAG_WILLCOPY)) {
          rc =
              wolfTPM2_HashUpdateCache(hashCtx, info->hash.in, info->hash.inSz);
        } else {
          ...
          if (rc == 0) {
            rc = wolfTPM2_HashUpdate(tlsCtx->dev, &hash, info->hash.in,
                                     info->hash.inSz);
          }
        }
      }
    ...
    if (info->hash.digest != NULL) { /* Final */
      ...
      if (hashCtx && (hashFlags & WC_HASH_FLAG_WILLCOPY)) {
        ...
        if (rc == 0) {
          rc = wolfTPM2_HashUpdate(tlsCtx->dev, &hash, hashCtx->cacheBuf,
                                   hashCtx->cacheSz);
        }
      }
      if (rc == 0) {
    ...
    if (info->hmac.in != NULL) { /* Update */
      ...
      if (hmacCtx == NULL) {
        ...
        rc = wolfTPM2_HmacStart(tlsCtx->dev, hmacCtx,
                                tlsCtx->storageKey ? &tlsCtx->storageKey->handle
                                                   : NULL,
                                hashAlg, keyBuf, keySz, NULL, 0);
      }
      if (rc == 0) {
        rc = wolfTPM2_HmacUpdate(tlsCtx->dev, hmacCtx, info->hmac.in,
                                 info->hmac.inSz);
      }
    }
